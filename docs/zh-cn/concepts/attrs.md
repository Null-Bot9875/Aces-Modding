...
| --- | --- | --- | --- |
| `attkShieldEx` | 金印 | 抵消一次受到的伤害，并移除提供该属性的 Skill | `CE9842-琉璃光SP` 的金印机制 |
| `stanceAdd` | 剑势 | 增加剑势相关伤害 | 枭首、振剑、奔袭势和核心剑装“敛锋”等剑势配置 |
| `deathAdd` | 黑矮星死亡标记 | 记录受击次数，供对应 Skill 读取 | 黑矮星战斗中的“垂死的银星”机制 |
| `boomAdd` | 倒计时自爆标记 | 记录自爆倒计时 | 威胁时序仪、延时引信的自爆机制 |
| `negaAdd` | 贬义叙述 | 修正“贬义叙述”的剩余反制次数 | 极境战斗中的“贬义叙述”机制 |
| `deadTrigger` | 复生 | 值为 Skill ID；机体死亡时执行该 Skill | `attr_def` 已定义，但没有可直接参考的现有 Static Skill 配置 |
| `attkBack` | 背刺 | 使攻击使用背刺目标规则 | 只在背刺测试配置中出现 |
| `relive` | 复生 | 机体死亡时触发一次复生，触发后移除该属性 | 只在复生测试配置中出现 |
| `noChoose` | 无法被选中 | 任何阵营都不能将该 Card 选为目标 | `attr_def` 已定义，但没有可直接参考的现有 Static Skill 配置 |

## 填写规则

- Attr 名称区分大小写，必须与表中的名称完全一致。
- 目标没有该属性时，不会产生移除结果。
- 目标有多个同名属性时，`event=110` 会将它们全部移除。
