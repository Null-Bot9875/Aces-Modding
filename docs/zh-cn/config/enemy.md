...
Chain 的回合模式、独立安装组、Condition、权重和表现字段见 [Chain 配置参考](chains.md)。
...
| `weight` | 同一行动池内的选择权重 |
| `useConditions` | Battle Condition ID；空表表示示例不增加额外条件 |
| `cardId` | Enemy 执行的已有 Card ID |
| `assetId` | 行动表现引用；示例复用已有值 |

Battle Condition 字段与组合规则见 [Battle Condition 配置参考](conditions.md#battle-condition)。已有 Card 的 Target 与 Effect 结算链可从 [Target](targets.md) 和 [Effect](effects.md) 参考继续追踪。
